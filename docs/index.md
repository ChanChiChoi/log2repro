# Documentation

> **[中文文档](../docs_zh/index.md)**

## Guides

- [Getting Started](getting-started.md) — Installation, first reproduction, configuration
- [User Guide](user-guide.md) — CLI reference, input formats, output structure, troubleshooting

## Reference

- [Parser Reference](parser-reference.md) — Supported log formats, regex rules, edge cases
- [Python Error Format Gaps](python-error-format-gaps.md) — Unsupported formats with examples & fix plan (D9)
- [LLM Prompt Design](llm-prompt-design.md) — System prompts, AST injection, refinement strategy
- [Evaluation Guide](evaluation.md) — 4 quality metrics, benchmark framework

## Development

- [Architecture](architecture.md) — 5-stage pipeline, module map, design decisions, data flow
- [Contributing](contributing.md) — Dev setup, testing, code style, PR process
- [Changelog](changelog.md) — Version history (D1–D7)
- [Roadmap](roadmap.md) — Milestones, completed & planned features
